...
checkButton.addEventListener("click", () => {
  ...
  if (inputValue == randomNum) {
    ...
    [guess.textContent, input.disabled] = ["أحسنت! تخمينك صحيح", true];
    ...
    [guess.textContent, remainChances.textContent] = ["خطأ , تخمينك اعلى من المطلوب", chance];
    guess.style.color = "#333";
    //Check if input value is < random number and within 1-99 range.
  } else if (inputValue < randomNum && inputValue > 0) {
    // Update the guessed number text and remaining chances
    [guess.textContent, remainChances.textContent] = ["خطأ , تخمينك اقل من المطلوب", chance];
    guess.style.color = "#333";
    // If the input value is not within the range of 1 to 99
  } else {
    // Update the guessed number text, color and remaining chances
    [guess.textContent, remainChances.textContent] = ["الرقم المدخل غير صحيح", chance];
  ...
  if (chance == 0) {
    ...
    [checkButton.textContent, input.disabled, inputValue] = ["اعادة اللعب", true, ""];
    [guess.textContent, guess.style.color] = ["  لقد خسرت, العب مرة اخرى", "#DE0611"];
  }
  if (chance < 0) {
